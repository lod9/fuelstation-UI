...
local pop = nil
local waiting = false
RegisterNetEvent('fuel-popui:drawtextuiwithinput')
AddEventHandler('fuel-popui:drawtextuiwithinput', function(table)
...
RegisterNetEvent('fuel-popui:showui')
AddEventHandler('fuel-popui:showui', function(table)
    if not open then
        local coord = GetEntityCoords(PlayerPedId())
        Wait(1000)
        open = false
...
RegisterNetEvent('fuel-popui:closeui')
AddEventHandler('fuel-popui:closeui', function(force)
