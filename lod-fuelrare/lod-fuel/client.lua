...
RegisterNetEvent('lod-fuel:open')
AddEventHandler('lod-fuel:open',function(vehicle,data)
	open(vehicle,data)
end)
...
RegisterNetEvent('lod-fuel:close')
AddEventHandler('lod-fuel:close',function()
	close()
end)
...
RegisterNetEvent("lod-fuel:syncfuel")
AddEventHandler("lod-fuel:syncfuel",function(index,change,FuelDecor)
...
RegisterNetEvent('lod-fuel:refuelFromPump')
AddEventHandler('lod-fuel:refuelFromPump',function(pumpObject,ped,vehicle)
...
RegisterNetEvent('lod-fuel:fuelevent')
AddEventHandler('lod-fuel:fuelevent',function(pumpObject,ped,vehicle)
	...
	if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped),vehicleCoords) < 3.5 then
		if isNearPump then
			...
			isFueling = true
			TriggerEvent('lod-fuel:refuelFromPump',isNearPump,ped,vehicle)
		end
	end
end)
...
function DrawtextUI(name,v,reqdist,event,arg,server,invehicle,key)
    ...
    TriggerEvent('fuel-popui:drawtextuiwithinput',table)
    local dist = #(v - GetEntityCoords(PlayerPedId()))
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped),vehicleCoords) < 3.5 then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle,-1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 99 and canFuel and isNearPump then
							DrawtextUI("Press [E] to Re Fuel Vehicle",stringCoords,3.5,'lod-fuel:open',{vehicle,output},false,false,'E')
				
						elseif canFuel and GetVehicleFuelLevel(vehicle) < 99 then
							DrawtextUI("Press [E] to Re Fuel Vehicle (PETROL CAN)",stringCoords,3.5,'lod-fuel:refuelFromPump',{false,ped,vehicle},false,false,'E')
...
RegisterNetEvent('lod-fuel:Notify')
AddEventHandler('lod-fuel:Notify',function(msg)
...
RegisterNUICallback('pay', function(data, cb)
	...
	if not paid then
		if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),GetEntityCoords(vehicle)) < 5 then
			TriggerServerEvent('lod-fuel:payfuel',math.floor(new_perc),false,VehToNet(vehicle),math.floor(new_perc),Config.FuelDecor,pumpLocation)
			paid = true
		end
	end
end)
